/* 
 * Bitcoin cryptography library
 * Copyright (c) Project Nayuki
 * 
 * https://www.nayuki.io/page/bitcoin-cryptography-library
 * https://github.com/nayuki/Bitcoin-Cryptography-Library
 */

package io.nayuki.bitcoin.crypto;

import static org.junit.Assert.assertArrayEquals;

import org.junit.Test;

/**
 * Tests the Keccak-256 hash function class.
 * 
 * @see Keccak256Test
 */
public final class Keccak256Test {

	@Test
	public void testBasic() {
		String[][] CASES = {
				// ASCII vectors
				{ "A", "C5D2460186F7233C927E7DB2DCC703C0E500B653CA82273B7BFAD8045D85A470", "" },
				{ "A", "4D741B6F1EB29CB2A9B9911C82F56FA8D73B04959D3D9D222895DF6C0B28AA15", "The quick brown fox jumps over the lazy dog" },
				{ "A", "578951E24EFD62A3D63A86F7CD19AAA53C898FE287D2552133220370240B572D", "The quick brown fox jumps over the lazy dog." },
				// Binary vectors
				{ "H", "6A769F93F255B078FE73AFF68F0422A279939920E4690B4AFF0E433CFA3D3DF3",
						"13BD2811F6ED2B6F04FF3895ACEED7BEF8DCD45EB121791BC194A0F806206BFFC3B9281C2B308B1A729CE008119DD3066E9378ACDCC50A98A82E20738800B6CDDBE5FE9694AD6D" },
				{ "H", "C06DD4261638C44AFCB186F0AF5DE20EA53AA63316FBB71728F874FF3DACEB0D",
						"1EED9CBA179A009EC2EC5508773DD305477CA117E6D569E66B5F64C6BC64801CE25A8424CE4A26D575B8A6FB10EAD3FD1992EDDDEEC2EBE7150DC98F63ADC3237EF57B91397AA8A7" },
				{ "H", "52CBC5DBE49B009663C43F079DD180E38A77533778062A72A29E864A58522922",
						"F13C972C52CB3CC4A4DF28C97F2DF11CE089B815466BE88863243EB318C2ADB1A417CB1041308598541720197B9B1CB5BA2318BD5574D1DF2174AF14884149BA9B2F446D609DF240CE335599957B8EC80876D9A085AE084907BC5961B20BF5F6CA58D5DAB38ADB" },
				{ "H", "3A8DFCFD1B362003DDFA17910727539E64B18021ABBA018B5F58D71F7A449733",
						"E35780EB9799AD4C77535D4DDB683CF33EF367715327CF4C4A58ED9CBDCDD486F669F80189D549A9364FA82A51A52654EC721BB3AAB95DCEB4A86A6AFA93826DB923517E928F33E3FBA850D45660EF83B9876ACCAFA2A9987A254B137C6E140A21691E1069413848" },
				{ "H", "BD6F5492582A7C1B116304DE28314DF9FFFE95B0DA11AF52FE9440A717A34859",
						"" },
				{ "H", "E717A7769448ABBE5FEF8187954A88AC56DED1D22E63940AB80D029585A21921",
						"" },
				{ "H", "344D129C228359463C40555D94213D015627E5871C04F106A0FEEF9361CDECB6",
						"" },
				{ "H", "4CE7C2B935F21FC34C5E56D940A555C593872AEC2F896DE4E68F2A017060F535",
						"" },
				// secp256k1 curve point public key vectors
				{ "H", "72F15D6555488541650CE62C0BED7ABD61247635C1973EB38474A2516ED1D884",
						"836B35A026743E823A90A0EE3B91BF615C6A757E2B60B9E1DC1826FD0DD16106F7BC1E8179F665015F43C6C81F39062FC2086ED849625C06E04697698B21855E" },
				{ "H", "500625E5A55B726C0FC56C62C2D7CF95645D33006175B78989035C7C9061D3F9",
						"EFB99D9860F4DEC4CB548A5722C27E9EF58E37FBAB9719C5B33D55C216DB49311221A01F638CE5F255875B194E0ACAA58B19A89D2E56A864427298F826A7F887" },
				{ "H", "6F68D681DE0407D447C30E14DD4ECCD742D17887F50C27AEBB14D99BFD7571B6",
						"C5BC5C47FFA548D0F2B63D4F05FA7064D72FA3657C287F38CC87140A639C9465D4C9EDEC42F06B631C443311FD91585BCAB873136EAC50AF6DAA5363F0405544" }, };

		for (String[] tc : CASES) {
			byte[] msg;
			switch (tc[0]) {
			case "A":
				msg = Utils.asciiToBytes(tc[2]);
				break;
			case "H":
				msg = Utils.hexToBytes(tc[2]);
				break;
			default:
				throw new AssertionError();
			}
			assertArrayEquals(Utils.hexToBytes(tc[1]), Keccak256.getHash(msg));
		}
	}

}
